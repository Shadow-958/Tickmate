...
const MyBookingsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      try {
        setLoading(true);
        const response = await apiClient.get('/api/tickets/my-bookings');

        const allBookings = response.tickets || [];
        const now = new Date();

        const upcoming = allBookings.filter(booking => 
          booking.eventId && new Date(booking.eventId.endDateTime) >= now
        );
        const past = allBookings.filter(booking => 
          booking.eventId && new Date(booking.eventId.endDateTime) < now
        );

        upcoming.sort((a, b) => new Date(a.eventId.startDateTime) - new Date(b.eventId.startDateTime));
