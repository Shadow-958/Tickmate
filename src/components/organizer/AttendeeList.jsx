            ...
            <table className="w-full text-left min-w-[640px]">
              <thead className="bg-gray-800/50">
                <tr>
                  ...
                  <th className="p-4 text-sm font-semibold text-gray-300">Booking Date</th>
                ...
                {attendees.map((ticket, index) => (
                  <tr key={ticket._id} className="border-b border-gray-800 hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="p-4 text-gray-300">
                      {ticket.attendeeId?.firstName || 'N/A'} {ticket.attendeeId?.lastName || ''}
                    </td>
                    <td className="p-4 text-gray-300">{ticket.attendeeId?.email}</td>
                    <td className="p-4 text-gray-300">
                      {new Date(ticket.bookingDate || ticket.createdAt).toLocaleDateString()}
                    </td>
                    <td className="p-4">
                      <StatusPill isCheckedIn={ticket.checkInStatus?.isCheckedIn || false} /> 
                    </td>
                  </tr>
                ))}
              </tbody>
